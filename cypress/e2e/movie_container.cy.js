describe('Movie Container user flows', () => {
  ...
  it('should make a call to the API to retrieve the data for all 40 movies', () => {
  ...
  it('should be able to see a backdrop image for a random movie, and a title that links to that movie\'s details', () => {
    cy.get('[class*=movie-container]')
    .should('be.visible')
    .get('[class*=random-backdrop]')
    .should('exist')
    .get('[class*=random-movie-title]')
    .should('exist')
    .click()
    .get('[class*=movie-info]')
    .should('be.visible')
  })
